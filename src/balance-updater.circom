...
template BalanceUpdater() {
    ...
    // compute fee2Charge
    ////////
    // fee is computed if tx is L2 and not NOP tx
    component computeTxFee = ComputeTxFee();
    computeTxFee.amount <== amount;
    computeTxFee.feeSelector <== feeSelector;
    computeTxFee.feeApplies <== (1-onChain)*(1-nop);

    computeTxFee.fee2Charge ==> fee2Charge;
